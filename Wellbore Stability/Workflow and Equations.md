# Workflow
This workflow is for extensional tectonic settings where $S_{V}$ = $\sigma_{1}$, $S_{Hmax}$ = $\sigma_{2}$, and $S_{hmin}$ = $\sigma_{3}$. To calculate $S_{V}$ we integrate the density log from TVDSS (h):
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMDY4ODkyMjcsLTY1OTExMDYwNiwtMT
M5MjEzMDcxMl19
-->