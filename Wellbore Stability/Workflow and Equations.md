# Workflow
This workflow is for extensional tectonic settings where $S_{V}$ = $\sigma_{1}$, $S_{Hmax}$ = $\sigma_{2}$, and $S_{hmin}$ = $\sigma_{3}$. To calculate $S_{V}$ we integrate the density log from $h = 0$ to $h = i+1$ where h = TVDSS (m) and i = next interva
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjExNjU2MTExOSwtMTM5MjEzMDcxMl19
-->